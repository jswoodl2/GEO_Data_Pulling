...
Auth:
  ...
  # will need to add your Key for this script to work
"""
...
# If you insist on hardcoding (not recommended), set FALLBACK_API_KEY below.
...
# Treat these as retryable transient statuses
...
def is_retryable_error(e: Exception) -> bool:
    ...
    if code is None:
        # When in doubt for transport errors, treat as retryable once
        txt = str(e).lower()
...
def call_model_with_retries(client_factory, prompt: str) -> Optional[str]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # Periodically recreate the client during failure bursts
            ...
            # Gentle cool-off after overload-type statuses
...
def main() -> None:
    ...
    # Factory so we can recreate clients during retry bursts
    def client_factory():
        return make_client()
    ...
    for idx, paper in enumerate(all_papers_data, start=1):
        ...
        # Auto-resume unless OVERWRITE=True
        ...
        # Base pacing only after *successful* attempts (errors already waited during backoff)
    ...
    # Try to merge into your template
    try:
    ...
    # Ensure columns are strings (Excel compatibility)
